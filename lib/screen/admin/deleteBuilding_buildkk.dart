...
class DelBuildKK extends StatefulWidget {
  @override
  _DelBuildKKState createState() => _DelBuildKKState();
}

class _DelBuildKKState extends State<DelBuildKK> {
  bool loadStatus = true;
  bool status = true;
  // ignore: deprecated_member_use
  List<CateBuildingModel> catebuildingModels = List();
  // ignore: deprecated_member_use
  List<CateBuildingModel> catebuildingModelss = List();
  String buildingStr;
          ...
          IconButton(
              onPressed: () {
                MaterialPageRoute materialPageRoute = MaterialPageRoute(
                    builder: (BuildContext context) => DelBuildKK());
          ...
          return Container(
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    Text('อาคาร ${catebuildingModels[index].buildKk}'),
                    IconButton(
  ...
  Future<Null> addBuildingThread() async {
    String url =
        '${Myconstant().domain}/appkk/addDatahouse_buildKK.php?isAdd=true&build_kk=$buildingStr&floorspp=0&numRoom=0';
    try {
    ...
    showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
              ...
              content: Text(
                'คุณต้องการลบอาคาร ${catebuildingModels[index].buildKk} ออกหรือไม่?',
              ),
              actions: <Widget>[
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget okButton(int index) {
    ...
    return FlatButton(
      onPressed: () {
        ...
        print(
            '1111111111111111111111111111111${catebuildingModels[index].buildId}');
  ...
  Future<Null> deleteData(idd) async {
    FirebaseMessaging firebase = FirebaseMessaging();
    String token = await firebase.getToken();
    print('token =========$token');
    String url =
        '${Myconstant().domain}/appkk/editDataWhereid.php?isAdd=true&build_kk='
        '&build_id=$idd';
    print('iiiiiiiiiiiiiiiiiiiiiiiiiiiiiii $idd');
    try {
      Response response = await Dio().get(url);
      print('----------$response');
    } catch (e) {}
